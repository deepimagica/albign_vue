...
<div class="container-fluid custom-data p-0">
    <div class="wizard_container p-0">
        ...
        <div class="div-aggrement p-5">
            ...
            <div class="border-1">
                <ul class="numeric-alphabet3 m-3 text-align-justify">
                    <li>Providing insights about improving outcome of therapy in the management of patients</li>
                    <li>Identification of therapy gaps and advise future course of direction </li>
                    <li>Provide consultation to Company on the trends in practice that will identify gaps in existing
                        therapeutic solutions and aid launch of newer therapeutic solutions to address the needs of
                        patients.</li>
                    <li>Providing details for further analysis and development of new products, based on trends, data
                        and statistics relying upon type of patients, illness, diseases etc., in specific format.
                    </li>
                </ul>
            </div>
        ...
        <div>
            <div class="body q-body">
                <div class="custom-header">
                    <h4 class="text-center heading"><b>Survey Questions</b></h4>
                </div>
                @if (!empty($answers))
                    @foreach ($answers as $key => $answer)
                        <h6 class="text-left question_mcq">
                            Q-{{ $key + 1 }}. {{ $answer->question }}
                        </h6>
                        <ul class="question-answer">
                            @php
                                $selectedAnswer = $answer->answer ?? '';
                                $options = explode(',', $answer->answers ?? '');
                            @endphp
                            ...
                            @if ($answer->type == 1)
                                ...
                                @foreach ($options as $option)
                                    <li>
                                        <span class="{{ trim($selectedAnswer) == trim($option) ? 'answer' : '' }}">
                                            {{ $option }}
                                            @if (trim($selectedAnswer) == trim($option))
                            ...
                            @elseif ($answer->type == 2)
                                {{-- Ranking question --}}
                                @php
                                    $rankedAnswers = array_map('trim', explode(',', $selectedAnswer));
                                @endphp
                                @foreach ($options as $option)
                                    <li>
                                        {{ $option }} - (Rank
                                        {{ array_search(trim($option), $rankedAnswers) + 1 }})
                                    </li>
                                @endforeach
                            ...
                            @elseif ($answer->type == 4)
                                {{-- Multiple choice --}}
                                @php
                                    $selectedOptions = array_map('trim', explode(',', $selectedAnswer));
                                @endphp
                                @foreach ($options as $option)
                                    <li>
                                        <span class="{{ in_array(trim($option), $selectedOptions) ? 'answer' : '' }}">
                                            {{ $option }}
                                            @if (in_array(trim($option), $selectedOptions))
                                                <i class="fa fa-check-circle-o" aria-hidden="true"></i>
                                            @endif
                                        </span>
                                    </li>
                            ...
                            @elseif ($answer->type == 5)
                                ...
                                @php $isOther = true; @endphp
                                @foreach ($options as $option)
                                    <li>
                                        <span class="{{ trim($selectedAnswer) == trim($option) ? 'answer' : '' }}">
                                            {{ $option }}
                                            @if (trim($selectedAnswer) == trim($option))
                                                @php $isOther = false; @endphp
                                                <i class="fa fa-check-circle-o" aria-hidden="true"></i>
                                            @endif
                                        </span>
                                    </li>
                                @endforeach
                                @if ($isOther)
                                    <li class="answer">Other - {{ $selectedAnswer }}</li>
                                @endif
                            @elseif (in_array($answer->type, [6, 7]))
                                {{-- Range-based questions --}}
                                @php
                                    $rangeValues = explode(',', $selectedAnswer);
                                @endphp
                                @foreach ($options as $index => $option)
                                    <li>{{ $option }} - {{ $rangeValues[$index] ?? 0 }}</li>
                                @endforeach
                            @endif
                        </ul>
                    @endforeach
